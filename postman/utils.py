...
def notify_user(object, action, site):
    from emis import sms
    from django.conf import settings
    s=sms.SmsGupshupSender(username=settings.SMS_GATEWAY_USERNAME, password=settings.SMS_GATEWAY_PASSWORD)
    ...
    if notification:
        ...
        if object.notify_via_sms:
            try:
                if label=='postman_message':
                    s.send(user.account.mobile_number,'You have received a message from %s at emis.tnschools.gov.in' % user.username)
                elif label == 'postman_reply':
                    s.send(user.account.mobile_number,'You have received a reply from %s at emis.tnschools.gov.in' % user.username)
                else:
                    pass    
